...
package daksha.ex.guiauto.model4.appFactoryWithPages;
...
public class WebTestWithSeleniumAutomator extends TestNGBaseTest{
	...
	public void createAutomator(ITestContext testContext) throws Exception {
		...
		App app = PageFactory.createAppFromDir("WordPress", builder.build(), "appwithpages");
	...
	public void test() throws Exception{
		...
		app.page("LeftNavigation").element("POSTS").hover();
		app.page("LeftNavigation").element("CATEGORIES").click();	
		app.waitForBody();
		
		MultiGuiElement tags = app.page("Categories").elements("CAT_CHECKBOXES");
		...
		app.page("LeftNavigation").element("SETTINGS").click();

		GuiElement blogNameTextBox = app.page("Settings").element("BLOG_NAME");
		blogNameTextBox.enterText("Hello");
		blogNameTextBox.enterText("Hello");
		blogNameTextBox.setText("Hello");
		
		app.page("Settings").element("MEMBERSHIP").check();

		// Experiments with Select control - Selection using different attributes
		GuiElement roleDropDown = app.page("Settings").element("ROLE").asDropDown();
