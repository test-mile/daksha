 ...
 ******************************************************************************/
package daksha.core.leaping.selenium;
...
public class SeleniumWebUiDriver extends DefaultUiDriver implements SeleniumUiDriver {
	...
	public UiElement declareElement(ElementMetaData elementMetaData) throws Exception {
		...
		SeleniumElementProxy proxy = createProxy(uiElement);
		uiElement.setProxy(proxy);
		uiElement.setLoaderType(this.getElementLoaderType());
		proxy.setFindersQueue(finderQueue);
		proxy.setAutomatorName(Batteries.getComponentName("WEBDRIVER_AUTOMATOR"));
		return uiElement;
	}
