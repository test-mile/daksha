	...
	public void process(Map<String, ConfigProperty> options, String option, Value value) throws Exception{
		...
		if (pathToEnumMap.containsKey(propPath)){
			...
			case BROWSER_BIN_PATH:
				...
				break;
			case BROWSER_PROXY_HOST:
				processStringOption(options, propPath, value, "Browser Proxy Host Name", true);
				break;
			case BROWSER_PROXY_ON:
				processBooleanOption(options, propPath, value, "Should enable proxy for Browser?", true);
				break;
			case BROWSER_PROXY_PORT:
				processIntOption(options, propPath, value, "Browser Proxy Port Number", true);
				break;
