...
import com.testmile.daksha.core.value.ValueFactory;
	...
	public static String pathToCoreAbsolutePath(String path) throws Exception {
		...
		if (path.startsWith("*")){
			if (path.startsWith("*/") || path.startsWith("*\\")){
				retPath = FileSystemUtils.getCanonicalPath(Trishanku.getRootDir() + path.substring(1));
			} else {
				retPath = FileSystemUtils.getCanonicalPath(Trishanku.getRootDir() + File.separator + path.substring(1));
			}
